...
**After running `php core/database/setup_settings.php`:**
```env
# --- MySQL Configuration ---
# DB_TYPE=mysql
...
# --- SQLite Configuration ---
DB_TYPE=sqlite
DB_DATABASE=database.db  # Will be created in core/storage/database/
...
→ Creating database directory: /path/to/core/storage/database
✓ Database directory created
✓ Connected to SQLite database: /path/to/core/storage/database/database.db
...
→ SQLite database location: /path/to/core/storage/database/database.db
...
chmod 755 core/storage/database
chmod 644 core/storage/database/*.db
```
...
- Run the setup script: `php core/database/setup_settings.php`
...
- Check that `/core/app/config/config.php` exists
...
#### SQLite:
```bash
sqlite3 core/storage/database/database.db < core/database/schema_sqlite.sql
```
...
- ✅ Keep database in `core/storage/database/` directory
- ✅ Add `core/storage/database/*.db` to `.gitignore`
...
cp core/storage/database/database.db backup.db

# Using SQLite backup command
sqlite3 core/storage/database/database.db ".backup backup.db"
```
...
**SQLite:**
```bash
cp backup.db core/storage/database/database.db
```
...
3. Check application logs in `/core/storage/logs/`
4. Enable debug mode in `/core/app/config/config.php`
