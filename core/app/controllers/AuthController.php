...
class AuthController
{
    ...
    public function login(): void
    {
        ...
        redirect($this->consumeIntendedUrl($user));
    }
    ...
    public function changePassword(): void
    {
        ...
        if ($success) {
            ...
            redirect($this->consumeIntendedUrl(Session::user()));
        } else {
    ...
    /**
     * Resolve and clear any stored post-login redirect target.
     */
    private function consumeIntendedUrl(?object $user = null): string
    {
        $target = normalize_internal_url($_SESSION['url.intended'] ?? null, '/');
        unset($_SESSION['url.intended']);

        if ($target === '/login' || $target === '/logout') {
            return '/';
        }

        if ($user && method_exists($user, 'isAdmin') && !$user->isAdmin() && str_starts_with($target, '/admin')) {
            return '/';
        }
    ...
    public function forgotPassword(): void
    {
        ...
        // Send reset email
        $resetLink = app_url('/reset-password?email=' . urlencode($email) . '&token=' . urlencode($token));
