...
class AdminController
{
    ...
    public function updateUser(string $id): void
    {
        ...
        if ($success) {
            if ($id === (int)($_SESSION['id'] ?? 0)) {
                ...
                if ($updatedUser) {
                    Session::regenerate();
                    Session::setSession($updatedUser);
                }
            }
    ...
    public function deleteUser(): void
    {
        ...
        try {
            ...
            if ($success && $user && $user->avatar) {
                $avatarPath = STORAGE_PATH . '/uploads/avatars/' . $user->avatar;
    ...
    public function showRegistrationSettings(): void
    {
        ...
        $hideUserContent = $this->settingsModel->isUserContentHidden();

        view('admin.registration-settings', [
            'registrationEnabled' => $registrationEnabled,
            'hideUserContent' => $hideUserContent
        ]);
    }
    ...
    public function updateSettings(): void
    {
        ...
        $data = [
            ...
            'show_get_in_touch_card' => isset($_POST['show_get_in_touch_card']),
        ];
    ...
    public function updateRegistrationSettings(): void
    {
        ...
        // Update registration-related settings
        $registrationEnabled = isset($_POST['registration_enabled']) ? true : false;
        $hideUserContent = !$registrationEnabled && isset($_POST['hide_user_content']);

        $registrationSuccess = $this->settingsModel->setRegistrationEnabled($registrationEnabled);
        $visibilitySuccess = $this->settingsModel->setUserContentHidden($hideUserContent);
        $success = $registrationSuccess && $visibilitySuccess;
    ...
    private function handleLogoUpload(array $file): ?string
    {
        ...
        if ($currentLogo && $currentLogo !== '/assets/img/logo.png') {
            $oldLogoRelativePath = preg_replace('#^/?assets/#', '', $currentLogo) ?? '';
            $oldLogo = asset_path($oldLogoRelativePath);
            if ($oldLogoRelativePath !== '' && str_starts_with($oldLogo, asset_path()) && file_exists($oldLogo)) {
                unlink($oldLogo);
            }
        }

        $imageResource = match ($actualMime) {
        ...
        imagecopyresampled($canvas, $imageResource, 0, 0, 0, 0, $width, $height, $width, $height);
        ...
        $saved = $useWebP
            ? imagewebp($canvas, $destination, 85)
            : imagepng($canvas, $destination, 6);

        imagedestroy($imageResource);
        imagedestroy($canvas);

        return $saved ? '/assets/img/' . $fileName : null;
    }
