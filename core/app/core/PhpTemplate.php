...
class PhpTemplate
{
    ...
    /**
     * Register a namespaced template path, e.g. employees::index.
     */
    ...
    {
        [$templatePath, $cacheName] = $this->resolveTemplatePath($template);
        ...
        try {
            $cacheKey = md5($cacheName . filemtime($templatePath));
    ...
    private function resolveTemplatePath(string $template): array
    {
        if (str_contains($template, '::')) {
            [$namespace, $name] = explode('::', $template, 2);
            if (isset($this->namespaces[$namespace])) {
                return [
                    $this->namespaces[$namespace] . str_replace('.', '/', $name) . '.view.php',
                    $namespace . '::' . $name,
                ];
            }
        }

        return [$this->templatesPath . $template . '.view.php', $template];
    }
