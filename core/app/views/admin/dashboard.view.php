...
function loadRegistrationSettings() {
    fetch('/admin/registration-settings')
        ...
        .then(html => {
            ...
            initializeRegistrationSettings();
        })
        .catch(error => {
...
function initializeRegistrationSettings() {
    const registrationToggle = document.getElementById('registration_enabled');
    const hideUserContentToggle = document.getElementById('hide_user_content');

    if (!registrationToggle || !hideUserContentToggle) {
        return;
    }

    const syncHideToggleState = () => {
        const registrationEnabled = registrationToggle.checked;
        hideUserContentToggle.disabled = registrationEnabled;

        if (registrationEnabled) {
            hideUserContentToggle.checked = false;
        }
    };

    registrationToggle.addEventListener('change', syncHideToggleState);
    syncHideToggleState();
}
</script>
</x-layout-app>
