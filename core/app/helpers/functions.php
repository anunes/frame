...
function asset_url(string $path = ''): string
{
    ...
    } elseif (str_starts_with($normalizedPath, 'assets/')) {
        $normalizedPath = substr($normalizedPath, strlen('assets/'));
    }

    $basePath = parse_url(defined('APP_URL') ? APP_URL : '', PHP_URL_PATH);
    ...
    return ($basePath === '' ? '' : $basePath) . '/assets/' . ltrim($normalizedPath, '/');
}
...
function normalize_internal_url(?string $url, string $default = '/'): string
{
    if (!is_string($url)) {
        return $default;
    }
    ...
    $trustedHost = parse_url(defined('APP_URL') ? APP_URL : '', PHP_URL_HOST);
    if (!$trustedHost || strtolower($parts['host']) !== strtolower($trustedHost)) {
        return $default;
    }
...
{
    $target = normalize_internal_url($_SERVER['HTTP_REFERER'] ?? null, '/');
    if (strncmp('cli', PHP_SAPI, 3) !== 0) {
        if (!headers_sent()) {
...
if (!function_exists('user_content_hidden')) {
    /**
     * Check if public user-facing account content should be hidden.
     *
     * This flag only applies when registration is disabled.
     */
    function user_content_hidden(): bool
    {
        static $hidden = null;
        ...
        if ($hidden === null) {
            $settings = new \app\models\Settings();
            $hidden = $settings->isUserContentHidden();
        }
...
if (!function_exists('user_content_visible')) {
    /**
     * Check if public user-facing account content should be visible.
     */
    function user_content_visible(): bool
    {
        return !user_content_hidden();
    }
}
...
if (!function_exists('site_logo')) {
    ...
    function site_logo(): string
    {
        ...
        return str_starts_with((string) $logo, '/assets/') || str_starts_with((string) $logo, 'assets/')
            ? asset_url((string) $logo)
            : (string) $logo;
    }
}
...
if (!function_exists('navbar_items')) {
    ...
    function navbar_items(string $section = 'main'): array
    {
        ...
        if ($config === null) {
            $configPath = APP_PATH . '/config/navbar.php';
        ...
        if ($section === 'main') {
            $items = navbar_insert_module_items($items);
        }
...
if (!function_exists('navbar_insert_module_items')) {
    /**
     * Insert module navbar items after Home in the main navbar.
     */
    function navbar_insert_module_items(array $items): array
    {
        $moduleItems = navbar_module_items();
        ...
        foreach ($items as $index => $item) {
            if (($item['active'] ?? '') === 'home' || strtolower((string) ($item['label'] ?? '')) === 'home') {
                $insertAfter = $index;
                break;
            }
        }
        ...
        if ($insertAfter === null) {
            return array_merge($moduleItems, $items);
        }

        return array_merge(
            array_slice($items, 0, $insertAfter + 1),
            $moduleItems,
            array_slice($items, $insertAfter + 1)
        );
    }
}

if (!function_exists('navbar_module_items')) {
    /**
     * Load visible module navbar items.
     */
    function navbar_module_items(): array
    {
        static $moduleItems = null;
        ...
        if ($moduleItems === null) {
            ...
            foreach ((array) $config as $module => $item) {
                if (!is_array($item) || !is_dir(BASE_PATH . '/files/' . $module)) {
                    continue;
                }
...
if (!function_exists('navbar_item_visible')) {
    ...
    function navbar_item_visible(array $item): bool
    {
        ...
        if (isset($item['show_if'])) {
            $conditions = is_array($item['show_if']) ? $item['show_if'] : [$item['show_if']];
            ...
            foreach ($conditions as $condition) {
                ...
                if ($condition === 'user_content_visible' && !user_content_visible()) {
                    return false;
                }
            }
        }
