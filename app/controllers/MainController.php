...
class MainController extends Controller
{
    ...
    public function submitContact()
    {
        ...
        try {
            $contactInfo = $this->settingsModel->getContactSettings();
            $recipientEmail = $contactInfo->email ?? MAIL_FROM_ADDRESS;

            $emailSent = $this->sendContactEmail($name, $email, $subject, $message);

            if ($emailSent) {
                SE::setflash("Thank you for contacting us! Your message has been sent to $recipientEmail. We will get back to you soon.", 'success');
            } else {
    ...
    private function sendContactEmail(string $name, string $email, string $subject, string $message): bool
    {
        try {
            ...
            $contactInfo = $this->settingsModel->getContactSettings();
            $companyEmail = $contactInfo->email ?? MAIL_FROM_ADDRESS;

            // Log the configuration for debugging
            error_log("Contact Form Email - Sending to: $companyEmail from: " . MAIL_FROM_ADDRESS);

            // Email settings - send to company
            $mail->setFrom(MAIL_FROM_ADDRESS, MAIL_FROM_NAME);
            $mail->addAddress($companyEmail, $contactInfo->company_name ?? 'Admin');
            ...
            $mail->send();
            error_log("Contact form email sent successfully to: $companyEmail");
            return true;
        } catch (Exception $e) {
